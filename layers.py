...
class FV(nn.Module):
    ...
    def __init__(self, hidden_size):
        ...
        #self.Wx =  Initialized_Conv1d(hidden_size, 1)
...
class RV_TAV(nn.Module):
    ...
    def forward(self, sketchy_prediction, log_p1, log_p2, max_len=15, use_squad_v2=True):
        ...
        not_answerable = 0.5 *  (1 - sketchy_prediction.to(device='cuda:0')) + 0.5 * span_answerable
        l_p1 = log_p1.clone()
        l_p2 = log_p2.clone()
        l_p1[not_answerable >= 0.45 ] = 0
        l_p2[not_answerable >= 0.45 ] = 0
