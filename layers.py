...
class StackedEncoder(nn.Module):
    ...
    def __init__(self, num_conv_blocks, kernel_size, num_heads=4, d_model=128, dropout=0.1, device="cuda:0"):
        ...
        self.self_attn_block = nn.MultiheadAttention(
            d_model, num_heads, dropout)
    ...
    def forward(self, x, mask, l, blks):
        ...
        for i, conv_block in enumerate(self.conv_blocks):
            ...
            x = self.layer_dropout(
                x, res, self.dropout * float(l) / total_layers)
            l += 1
        res = x
        ...
        x = x.transpose(1, 2)
        x = x.permute(1, 0, 2)
        x, attn_output_weights = self.self_attn_block(
...
class RV_TAV(nn.Module):
    ...
    def forward(self, sketchy_prediction, intensive_prediction, log_p1, log_p2, max_len=15, use_squad_v2=True):
        ...
        has = torch.tensor([s_in[x, starts[x]] * e_in[x, ends[x]]
